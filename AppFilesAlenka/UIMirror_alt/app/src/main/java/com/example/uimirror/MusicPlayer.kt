...
class MusicPlayer(private val context: Context) {
    ...
    private val database = Room.databaseBuilder(
        ...
        PersonDatabase::class.java,
        "person_database"
    ).fallbackToDestructiveMigration() // Falls die Datenbankversion sich ändert
        .build()

    private val coroutineScope = CoroutineScope(Dispatchers.Main)

    // Methode um den Primary User aus der Datenbank zu laden
    private suspend fun getPrimaryUser(): Person? {
        return database.personDao().getPrimaryUser(true)
    }

    // Speichert die ID des ausgewählten Songs in der Datenbank
    fun saveSelectedSongId(songId: Int) {
        coroutineScope.launch {
            val primaryUser = getPrimaryUser()  // Holt den Primary User
            primaryUser?.let {
                // Speichert die Song-ID und Position des Songs für den Primary User
                database.personDao().updateSongData(it.id, songId, 0) // 0 ist der Startpunkt der Position
            }
        }
    }

    // Lädt die ID des zuletzt ausgewählten Songs aus der Datenbank
    fun loadSelectedSongId(): Int? {
        var songId: Int? = null
        coroutineScope.launch {
            val primaryUser = getPrimaryUser()  // Holt den Primary User
    ...
    fun playMainSong() {
        coroutineScope.launch {
            val newSongId = loadSelectedSongId()  // Lädt den aktuell gespeicherten Song aus der Datenbank
            Log.e("playMainSong", "songId ${newSongId} CurrentMainSongId ${currentMainSongId}")
            if (newSongId != null && newSongId != currentMainSongId) {
                mainPlayer?.release() // Wenn ein neuer Song geladen wird, vorherigen Player loslassen
                currentMainSongId = newSongId
                playSong(newSongId, isPreview = false) // Starte den neuen Hauptsong
            } else if (newSongId != null) {
                resumeMainSong() // Falls der Song derselbe ist, wird die Wiedergabe fortgesetzt
            }
        }
    ...
    // Stoppt die Vorschau und resume den Hauptsong, wenn er vorher gespielt wurde
    fun stopPreview() {
        handler.removeCallbacksAndMessages(null) // Entferne alle geplanten Aufgaben
        previewPlayer?.apply { // Wenn der Vorschau-Player existiert
            if (isPlaying) { // und gerade spielt,
                stop() // stoppe die Wiedergabe
            }
    ...
    // Pausiert den Hauptsong und speichert die Position in der Datenbank
    fun pauseMainSong() {
        mainPlayer?.let {
            currentPosition = it.currentPosition // Speichere die aktuelle Position
            coroutineScope.launch {
                val primaryUser = getPrimaryUser()  // Hole den Primary User
                primaryUser?.let {
                    // Speichere die Position des Songs in der Datenbank
                    database.personDao().updateSongData(it.id, currentMainSongId, currentPosition)
                }
            }
            it.pause() // Pausiere den Song
